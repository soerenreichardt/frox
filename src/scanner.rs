...
impl Scanner {
    ...
    fn scan_token(&mut self, substring: &str) -> Option<Result<TokenType>> {
        match substring {
            " " | "\r" | "\t" => {
                None
            },
    ...
    fn advance(&mut self) -> Rc<str> {
        self.current += 1;
        self.context.source.get(self.current-1..self.current).unwrap().into()
    }
    ...
    fn parse_operator(&mut self, substring: &str) -> Option<Result<TokenType>> {
    ...
    fn is_digit(substring: &str) -> bool {
        substring.parse::<f64>().is_ok()
    }

    fn is_letter(substring: &str) -> bool {
        let c = substring.chars().next().unwrap();
        c.is_alphabetic() || c == '_'
    }

    fn is_alpha_numberic(substring: &str) -> bool {
...
mod tests {
    ...
    fn should_parse_comment() {
        let mut scanner = Scanner::new("//some comment".into());
        let tokens = scanner.scan_tokens().unwrap();
        let token_types = tokens.iter()
            .map(|token| &token.token_type)
            .collect::<Vec<_>>();
    ...
    fn should_ignore_whitespace_with_comment() {
        let mut scanner = Scanner::new("(( )){} // grouping stuff".into());
        let tokens = scanner.scan_tokens().unwrap();
        let token_types = tokens.iter()
            .map(|token| &token.token_type)
            .collect::<Vec<_>>();
    ...
    fn should_parse_string_literals() {
        let mut scanner = Scanner::new("\"literally a string\"".into());
        let tokens = scanner.scan_tokens().unwrap();
    ...
    fn should_parse_identifiers_and_keywords() {
        ...
        assert_eq!(expected, token_types);
        assert_eq!("foo", tokens.get(3).unwrap().lexeme.materialize(&Context::new(source.into())))
    }
